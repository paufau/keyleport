...
namespace net
{
  ...
  {
  public:
    std::unique_ptr<Sender> createSender(const std::string &ip, int port) override
    {
      return make_sender(ip, port);
    }
    std::unique_ptr<Receiver> createReceiver(int port) override
    {
      return make_receiver(port);
    }
  };
