...
{
  if (discovery_) return discovery_->on_discovery;
  return discovery_fallback_on_discovery_;
}

event_emitter<peer_info>& app_net::on_lose()
{
  if (discovery_) return discovery_->on_lose;
...
{
  std::lock_guard<std::mutex> lock(mtx_);
  if (!current_conn_) return;
  auto peer = current_peer_;
  current_conn_->disconnect();
  clear_receive_handler_();
  current_conn_.reset();
  current_peer_.reset();
  if (peer) on_session_end_.emit(*peer);
}
...
{
  std::lock_guard<std::mutex> lock(mtx_);
  if (current_conn_) current_conn_->send_reliable(data);
}

void app_net::send_unreliable(const std::string& data)
{
  std::lock_guard<std::mutex> lock(mtx_);
  if (current_conn_) current_conn_->send_unreliable(data);
}
...
void app_net::bind_receive_handler_()
{
  clear_receive_handler_();
  if (current_conn_) {
    recv_sub_id_ = current_conn_->on_receive_data.subscribe([this](const network_event_data& ev){
      if (on_receive_) on_receive_(ev.data);
    });
  }
}
