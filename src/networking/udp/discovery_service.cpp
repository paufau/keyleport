...
discovery_service::discovery_service(udp_service& net)
  : net_(net)
{
}
...
void discovery_service::start_discovery(int listen_port, std::chrono::seconds lose_after)
{
  ...
  net_.on_receive_broadcast.subscribe([this](const network_event_broadcast& ev) {
    peer_info info;
    ...
    if (is_new) on_discovery.emit(info);
  });
...
void discovery_service::loop_expire_()
{
  while (running_) {
    ...
    for (auto& p : lost) on_lose.emit(p);
