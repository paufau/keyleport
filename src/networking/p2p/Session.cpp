...
void Session::set_on_message(std::function<void(const std::string&)> cb) { on_message_ = std::move(cb); }

void Session::start_server(std::function<void()> on_ready) {
...
void Session::start_client(std::function<void()> on_ready) {
  auto self = shared_from_this();
  write_line("hello\n",
    [this, self, on_ready](std::error_code ec, std::size_t) {
      if (ec) {
