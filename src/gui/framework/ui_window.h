...
namespace gui
{
  namespace framework
  {
    ...
    public:
      ...
      // Set or replace the key scene. Implementations should call willUnmount
      // on the previous scene (if any) and didMount on the new one.
      virtual void setScene(UIScene* scene) = 0;

      // Constrain the mouse to this window (implementation-defined behavior)
      // Used by scenes that need locked cursor or capture, e.g., for
      // drag/resize or relative mouse input.
    ...
    // Deinitializes and destroys the concrete window instance and its
    // resources. Safe to call multiple times; no-op if not initialized.
    void deinit_window();

    // Set or replace the key scene on the window singleton
    void set_window_scene(UIScene* scene);

    // Convenience: set scene by type. Holds a static instance per T for the app
    // lifetime.
    template <typename T, typename... Args>
    void set_window_scene(Args&&... args)
    {
    ...
    // Get a reference to the window singleton. Initializes the window if
    // needed.
    UIWindow& get_window();
